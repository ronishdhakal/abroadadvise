...
const ConsultancyGallery = ({ formData, setFormData }) => {
  ...
  useEffect(() => {
    if (formData.gallery_images && formData.gallery_images.length > 0) {
      const loadedImages = formData.gallery_images.map((img) => ({
        id: img.id || Date.now(), // ✅ Preserve existing image IDs
        image: typeof img === "string" ? img : img.image, // ✅ Show URL for old images
        file: img.file || null,
        isNew: !img.id, // ✅ Identify new images
      }));
  ...
  const handleFileChange = (e) => {
    ...
    const newImages = files.map((file) => ({
      ...
      image: URL.createObjectURL(file), // ✅ Preview newly uploaded images
      file, // Store actual file for FormData submission
      isNew: true,
    }));
    ...
    setFormData((prev) => ({
      ...prev,
      gallery_images: [...prev.gallery_images, ...newImages], // Append new images
    }));
  };
  ...
  const handleDeleteImage = (index) => {
    ...
    // ✅ Handle deleting existing images (track IDs for backend)
    const updatedImages = previewImages.filter((_, i) => i !== index);
    setPreviewImages(updatedImages);
    setFormData((prev) => ({
      ...prev,
