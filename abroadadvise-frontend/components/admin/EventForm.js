...
const EventForm = ({ eventSlug, onSuccess, onCancel }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    // ✅ Ensure correct conversion for slug-based fields
    const formattedData = {
        ...formData,
        targeted_destinations: JSON.stringify(formData.targeted_destinations || []), // Slugs instead of IDs
        related_universities: JSON.stringify(formData.related_universities || []), // Slugs instead of IDs
        related_consultancies: JSON.stringify(formData.related_consultancies || []), // Slugs instead of IDs
        organizer_slug: formData.organizer_slug || "", // Organizer slug
    };

    // ✅ Log the final data before submission
    console.log("📤 Final FormData before API call:", formattedData);
