...
const DestinationHeader = ({ destination = {}, setIsModalOpen, setSelectedEntity }) => {
  ...
  const handleInquiry = () => {
    if (!setSelectedEntity || !setIsModalOpen || !destination?.id || !destination?.title) {
      console.error("Missing data for inquiry modal.");
      return;
    }
  ...
  return (
    <div className="relative w-full flex flex-col justify-end overflow-hidden">
      ...
      >
        {/* Destination Logo + Info */}
        <div className="flex items-center gap-5 w-full sm:w-auto">
          {destination?.country_logo && (
            <div className="relative -mt-16 sm:-mt-24 w-24 h-24 sm:w-28 sm:h-28 bg-white p-2.5 rounded-2xl shadow-xl border-4 border-white hover:shadow-2xl hover:-translate-y-1 transition-all">
              <img
          ...
          <div className="flex-1 pt-2 sm:pt-0">
            ...
            <div className="flex items-center text-gray-500 text-sm md:text-base mt-2">
              <Globe className="h-5 w-5 md:h-6 md:w-6 mr-2 text-gray-400" />
              <span>International Education Destination</span>
            </div>
          </div>
        </div>
