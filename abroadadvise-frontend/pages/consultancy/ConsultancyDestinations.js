        ...
        {visibleDestinations.map((dest) => (
          ...
          >
            <Link href={`/destination/${dest.slug || "#"}`} className="flex items-center gap-3 w-full">
              ...
              />
              <span className="text-gray-800 font-medium">{dest.title || "Untitled Destination"}</span>
            </Link>
            ...
            {verified && (
              <button
                onClick={() =>
                  openInquiryModal("destination", dest.id, dest.title, consultancyId, consultancyName)
                }
