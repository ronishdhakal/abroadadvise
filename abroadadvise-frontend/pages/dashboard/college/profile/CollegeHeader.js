...
import { updateCollegeDashboard } from "@/utils/api"; // ✅ Make sure you have this in your API utilities
...
const CollegeHeader = ({ formData, setFormData }) => {
  ...
  const handleInputChange = (e) => {
    ...
    // Preserve study_abroad_destinations and university fields during updates
    if (name === "study_abroad_destinations" || name === "university") {
      setFormData((prev) => ({
        ...prev,
        [name]: value, // Maintain the current value for these fields
      }));
    } else {
      setFormData((prev) => ({
        ...prev,
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      // Preserve study_abroad_destinations and university fields
      ...
      if (formData.university) {
        updateData.append("university", formData.university);
      }
      ...
      setSuccessMessage("College header updated successfully!");
    } catch (err) {
